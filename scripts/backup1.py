...
class CurveTracer:
    ...
    def goto(self, dest_x, dest_y):
        ...
        while self.state != Bot_State.Goal_Reached.value:         # when particular point goal is not reached
            theta_error = self.utils(dest_x,dest_y)
            print("theta error: "+ str(theta_error))
             
           
            if self.state == Bot_State.Fixing_Yaw.value:          # if statement to fix yaw of bot

                
                
              while(np.abs(theta_error) > self.theta_precision):  # to check whether generated errror is in within limit or not
                ...
                theta_error = self.utils(dest_x,dest_y)
                self.fix_error(self.P,0,theta_error)              # if not then call function to correct it
